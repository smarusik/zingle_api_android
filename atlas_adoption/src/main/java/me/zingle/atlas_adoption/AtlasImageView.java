...
import me.zingle.atlas_adoption.utils.ViewUtils;
...
public class AtlasImageView extends View {
    ...
    protected void onMeasure(int widthSpec, int heightSpec) {
        ...
        if (debug) Log.w(TAG, "onMeasure() before: " + mWidthBefore + "x" + mHeightBefore
                + ", spec: " + ViewUtils.toStringSpec(widthSpec, heightSpec)
        ...
        if (widthMode == MeasureSpec.EXACTLY && heightMode == MeasureSpec.EXACTLY) {
            if (debug) Log.w(TAG, "onMeasure() exact dimenstions, skipping " + ViewUtils.toStringSpec(widthSpec, heightSpec));
        } else if (widthMode == MeasureSpec.UNSPECIFIED && heightMode == MeasureSpec.UNSPECIFIED) {
            if (debug) Log.w(TAG, "onMeasure() first pass, skipping " + ViewUtils.toStringSpec(widthSpec, heightSpec));
            ...
            if (debug) Log.w(TAG, "onMeasure() unchanged. " + ViewUtils.toStringSpec(widthSpec, heightSpec));
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (iOSBug) {
            switch (orientation) {
                case ORIENTATION_1_CW_180  : canvas.rotate(180, 0.5f * drawable.getBounds().width() , 0.5f * drawable.getBounds().height()); break;
                case ORIENTATION_2_CW_90    :
                    if (false) {
            ...
            if (orientation == ORIENTATION_3_CCW_90 || orientation == ORIENTATION_1_CW_180) {
                drawable.setBounds(0,0, viewHeight, viewWidth);
            }
            switch (orientation) {
                case ORIENTATION_1_CW_180  : canvas.rotate(-90, 0.5f * drawable.getBounds().width() , 0.5f * drawable.getBounds().height()); break;
                case ORIENTATION_2_CW_90    : canvas.rotate(180, 0.5f * viewWidth , 0.5f * viewHeight); break;
                case ORIENTATION_3_CCW_90 : canvas.rotate(90,  0.5f * drawable.getBounds().width() , 0.5f * drawable.getBounds().height()); break;
                default: canvas.rotate(angle, 0.5f * viewWidth , 0.5f * viewHeight);
            }
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (debug) Log.w(TAG, "onTouch() event: " + ViewUtils.toString(event));
